...
1. 只修改输出目录中的工作簿副本，不修改 `策划/配置表/Table` 源表或其他正式文件。
2. 草案位置由目标 Sheet schema 和副本结构决定，不强制写在 A 列 `END` 下方；若存在 `END`，在副本中保留其原位和导出截止语义。
3. 草案必须明确标注“副本草案/尚未导出”，并按正式配置标准填写完整字段、合法 JSON、候选 ID、外键和 `tlanguage_cn` 引用。
4. 需要进入正式区时，只能由人工或项目发布流程在确认后处理；AI 不移动、覆盖或写入正式文件。
5. 若目标 Sheet 的写入边界或 schema 不明确，AI 应在副本中标记“位置待确认”，而不是猜测并写入正式文件。
