...
默认先生成测试副本并保留源表不变。AI 新增或修改的事件、剧情、对话、文本和依赖配置，统一写在各目标 Sheet 的 **A 列 `END` 行下方**，并保持 `END` 行不动。该区域不参与导出，属于待确认草稿区。

即使位于 `END` 下方，配置仍须满足正式字段结构、ID 构成、JSON、外键和 `tlanguage_cn` 引用规则。只有用户明确要求“确认并转为正式配置”时，才把确认过的行移动到 `END` 上方；移动后必须重新执行唯一性、引用完整性、公式缓存和试导出 QA。目标 Sheet 如果没有 A 列 `END` 行，AI 应停止该 Sheet 的写入并报告“缺少行 END，待确认”，不能擅自追加到正式数据末尾。
...
9. 所有新增/修改配置先写在目标 Sheet 的 A 列 END 行下方；未收到“确认并转为正式配置”指令前，不得移到 END 上方。
```
