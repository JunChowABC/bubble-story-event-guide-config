...
- 用户未明确要求激活时，生成的配置仍遵守第 7.9 节：只写两张正式表各自 `END` 下方的草案区。
...
`END` 是正式导出截止标记。`END` 上方为正式导出区，`END` 所在行必须保持原位，`END` 下方为 AI 草案区；草案默认不参与导出，也不代表已经在游戏中生效。

> 强制流程：正式数据 → `END` → AI 草案 → 用户审核 → 明确授权后再激活。

对 `tGuideLogic` 和 `tGuideGroup` 必须分别执行以下规则，不得假设两张表的 `END` 在相同行号：

1. 在 A 列中定位第一个单元格内容严格等于 `END` 的行，AI 从该行的下一行开始写入。
2. 不得编辑、移动、删除或覆盖原 `END`；不得新增第二个 `END`；不得在 `END` 上方的空白行插入 AI 配置。
3. `tGuideLogic` 草案按 A 列备注、B:AW 配置字段写入；`tGuideGroup` 草案按 A 列备注、B:AD 配置字段写入。
4. 新增草案的 A 列统一写成 `[AI草案][新增][A-假设] 说明`。如值已由代码或关联表验证，可把 `A-假设` 改成对应来源标记。
5. 修改已有配置时，不直接改正式行，而是把原行完整复制到 `END` 下方后修改，A 列写成 `[AI草案][修改原行xx][A-假设] 说明`。需要改前置组 `nextId` 时，也复制该前置组到草案区提出修改。
6. 格式、公式和数据验证优先从 `END` 上方最接近的同类型成熟配置复制，不得只因为它紧邻 `END` 就盲目套用。
7. 新增 ID 必须在正式区和草案区合并检查后仍然唯一。修改草案可暂时沿用原 ID，但激活时必须替换原正式行，不能让同 ID 的两行同时进入正式区。
8. 草案 `tGuideGroup.guides` 可以引用草案 `tGuideLogic.id`；静态 QA 按“正式区 + 草案区”解析引用，但交付说明必须标明这些引用尚未导出、生效。
9. 只有用户明确要求“激活/写入正式区”时，才可把审核通过的草案复制到 `END` 上方，或由人工把 `END` 下移。默认任务只生成 `END` 下方草案。

### 7.10 实际配置顺序

1. **确定源与输出副本**：读取当前 `Table/Y_引导表_tGuideGroup.xlsx`，另存新版本；禁止原地覆盖源表。
2. **分别定位截止行**：在 `tGuideLogic` 和 `tGuideGroup` 的 A 列分别找到首个严格等于 `END` 的单元格，记录草案起始行为 `END 行 + 1`。
...
6. **再配 `tGuideLogic` 草案**：从其 `END` 下一行开始，每个玩家可感知步骤一行；写严格 JSON；验证路径与 EventAdaptor；明确完成条件。
7. **再配 `tGuideGroup` 草案**：从其 `END` 下一行开始，写触发、附加界面条件、恢复策略，并把逻辑 ID 按顺序写进 `guides`。
8. **提出接链修改**：若需修改前组 `nextId`，把前组复制到 `tGuideGroup` 的草案区再修改；不得直接改正式前组。若使用 `keepEvent=9`，保证它确实由前组推进。
9. **静态 QA**：ID 唯一、引用存在、数组可解析、数组长度匹配、`itemLimit` 偶数、类型字段匹配；两张表的原 `END` 不动，全部 AI 行严格位于各自 `END` 下方。额外检查每个点击目标的代码调用结果是否真是需求功能，以及 `keepEvent` 所需参数是否齐全。
10. **文案与逻辑 QA**：有对话时先建立“界面/物品当前状态、角色已知信息、本场已提及话题”，写完执行 logic-guard；不能只检查错别字和口癖。
11. **运行 QA**：仅在草案被明确授权激活后执行；先用真实事件验证组能触发，再用新存档和中断存档各测一次，验证遮罩不会死锁输入。跨界面链必须逐段验证“上一组完成→下一组进入候选→目标界面事件→对话/点击完成”。
...
- 不得把新增或修改草案写进 `END` 上方的正式区或空白行；不得移动原 `END`、添加第二个 `END`，或在用户未明确授权时激活草案。
...
- 不覆盖源表，输出一个新 xlsx；保留 6 行表头、样式、公式、图片、隐藏状态和 END。
